...
class AdapterForCreateMenuDayMeal(private val listener: AdapterForCreateMenuDayMeal.OnItemClickListener, var menuTemp: List<MenuTemp>  ): RecyclerView.Adapter<AdapterForCreateMenuDayMeal.CreateMenuDayMealViewHolder>() {
    ...
    override fun onBindViewHolder(holder: CreateMenuDayMealViewHolder, position: Int) {
        ...
        if (menuTemp[position].calendarDay.equals(0L) == false ){holder.itemView.textViewShowDate.setText(SimpleDateFormat("dd/M/yyyy").format(menuTemp[position].calendarDay))} else
